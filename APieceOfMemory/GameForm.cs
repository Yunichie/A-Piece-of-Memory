...
namespace APieceOfMemory
{
    public partial class GameForm : Form
    {
            ...
            switch (level)
            {
                ...
                case 1: case 2: case 3: case 4:
                    if (level == 1)
                    {
                        waterGoal = 3; 
                        fertilizerGoal = 2; 
                        enemySpawnInterval = TimeSpan.FromSeconds(2.0);
                    }
                    ...
                    if (level == 3)
                    {
                        waterGoal = 8; 
                        fertilizerGoal = 5; 
                        enemySpawnInterval = TimeSpan.FromSeconds(1.0);
                    }
                    ...
                    if (level == 4)
                    {
                        waterGoal = 12; 
                        fertilizerGoal = 8; 
                        enemySpawnInterval = TimeSpan.FromSeconds(0.7);
                    }
        ...
        private void UpdateGame()
        {
            ...
            {
                if (!playerProjectiles[i].Update(ClientRectangle)) playerProjectiles.RemoveAt(i);
            }

            for (int i = enemyProjectiles.Count - 1; i >= 0; i--)
            {
                if (!enemyProjectiles[i].Update(ClientRectangle)) enemyProjectiles.RemoveAt(i);
            }

            for (int i = activeCollectibles.Count - 1; i >= 0; i--)
            {
                activeCollectibles[i].Update(); 
                if (activeCollectibles[i].IsExpired) activeCollectibles.RemoveAt(i);
            }
                 ...
                 enemies.Count < (currentLevel == 4 ? 12 : currentLevel == 6 ? 10 : 8)) // Adjusted max enemies 
            {
                ...
                if ((currentLevel >= 1 && currentLevel <= 4) && player != null)
                {
                ...
                if (currentLevel >= 1 && currentLevel <= 4)
                {
                    spawnX = this.ClientSize.Width + random.Next(20, 120);
                }
                else
                {
                ...
                if (spawnX > targetForEnemy.X) newEnemy.Speed = -Math.Abs(newEnemy.Speed); 
                else newEnemy.Speed = Math.Abs(newEnemy.Speed);
            ...
            for (int i = enemies.Count - 1; i >= 0; i--) 
            {
                ...
                if (enemies[i].Bounds.IntersectsWith(flower.Bounds)) 
                {
                    ...
                    {
                        HandleGameOver("The memory faded... The flower is gone."); 
                        return;
                    }
                } 
            ...
            if (boss != null && currentLevel == 5) {
                ...
                if (isBossPerformingSpecialAttack) 
                {
                    if (DateTime.Now < bossSpecialAttackEndTime) 
                    {
                        ...
                        { // Increased chance/rate of special projectiles
                ...
                else 
                {
                    ...
                    {
                        int projectilesInSpread = random.Next(3,8); 
                        float totalSpreadAngleDegrees = random.Next(25,75); 
        ...
        private void CheckCollisions() 
        {
            ...
            for (int i = playerProjectiles.Count - 1; i >= 0; i--) 
            {
                ...
                {
                    if (i < playerProjectiles.Count && playerProjectiles[i].Bounds.IntersectsWith(enemies[j].Bounds)) 
                    { // Check i validity
                        enemies[j].Health--;
            ...
            if (boss != null && currentLevel == 5) 
            {
                for (int i = playerProjectiles.Count - 1; i >= 0; i--) 
                {
                    if (i < playerProjectiles.Count && playerProjectiles[i].Bounds.IntersectsWith(boss.Bounds)) 
                    { // Check i validity
                        ...
                        if (boss.Health <= 0)
                        {
                            ...
                            boss = null; /* Completion handled by CheckLevelCompletion */
                        }
                        break; 
                    }
                }
            ...
            if (currentLevel >= 1 && currentLevel <= 4) 
            {
                ...
                for (int k = activeCollectibles.Count - 1; k >= 0; k--) 
                {
                    ...
                    if (collectionZone.IntersectsWith(activeCollectibles[k].Bounds)) 
                    {
                        ...
                        if (collected.Type == CollectibleType.Water)
                        {
                            waterProgress++; 
                            temporaryFeedbackMessage = "+1 Water!";
                        }
                        else
                        {
                            fertilizerProgress++; 
                            temporaryFeedbackMessage = "+1 Fertilizer!";
                        }
            ...
            for (int i = enemyProjectiles.Count - 1; i >= 0; i--) 
            {
                if (i < enemyProjectiles.Count && enemyProjectiles[i].Bounds.IntersectsWith(player.Bounds)) 
                { // Check i validity
                    enemyProjectiles.RemoveAt(i); flower.TakeDamage();
                    if (flower.State == FlowerState.Dead)
                    {
                        HandleGameOver("You couldn't protect the memory... It shattered."); 
                        return;
                    }
                    else
                    {
            ...
            for (int i = enemies.Count - 1; i >= 0; i--)
            {
                if (i < enemies.Count && player.Bounds.IntersectsWith(enemies[i].Bounds)) 
                { // Check i validity
                    enemies.RemoveAt(i); flower.TakeDamage();
                    if (flower.State == FlowerState.Dead)
                    {
                        HandleGameOver("The chaos was too much for the fragile memory."); 
                        return;
                    }
                    else
                    {
        ...
        private void CheckLevelCompletion() 
        {
            ...
            if (conditionsMet)
            {
                currentScreenState = GameScreenState.LevelTransition; 
        ...
        private void DrawGamePlayingState(Graphics g) {
            ...
            foreach (var p in playerProjectiles) p.Draw(g); 
            foreach (var c in activeCollectibles) c.Draw(g);
            foreach (var en in enemies) en.Draw(g); 
            if (boss != null) boss.Draw(g);
            foreach (var ep in enemyProjectiles) ep.Draw(g);
            
            // UI Text
            string lt = $"Level: {currentLevel}"; 
            g.DrawString(lt, gameFont, Brushes.LightGray, 10,10);
            string fst = $"Flower: {flower.State}"; 
            g.DrawString(fst, gameFont, Brushes.LightGray, 10, 10+gameFont.Height+2);
            string ot = ""; 
            if (currentLevel==1) ot = $"Care (E/R): W:{waterProgress}/{waterGoal} F:{fertilizerProgress}/{fertilizerGoal}"; else if (currentLevel >=2 && currentLevel <=4) ot = $"Collect: W:{waterProgress}/{waterGoal} F:{fertilizerProgress}/{fertilizerGoal}";
        ...
        {
            using (SolidBrush overlayBrush = new SolidBrush(Color.FromArgb(200, 0, 0, 0)))
            {
                g.FillRectangle(overlayBrush, 0, ClientSize.Height / 3.5f, ClientSize.Width, ClientSize.Height / 2.5f);
            }
            ...
            {
                Rectangle rectPrompt = new Rectangle(0, (int)(rectMessage.Bottom - 70), ClientSize.Width, 60); TextRenderer.DrawText(g, prompt, gameFont, rectPrompt, Color.LightYellow, flags);
            }
        }
            ...
            switch (currentScreenState) 
            {
                case GameScreenState.Playing:
                    ...
                    if (e.KeyCode == Keys.Q)
                    {
                        ...
                        if (currentLevel > 6)
                        {
                            HandleGameOver("DEBUG: All levels skipped.");
                        }
                        else
                        {
                            StartGameplayLevel(currentLevel);
